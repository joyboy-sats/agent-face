...
为 agent、钱包地址和匿名身份生成可复现的 SVG 多角色头像。

![AgentFace 头像阵列](./docs/images/avatar-grid.svg)

AgentFace 可以把钱包地址、公钥、哈希或任意稳定字符串，映射成一张稳定、可复现、适合产品界面使用的多角色头像。
...
- 输入 `seed`、地址或任意稳定标识符
- 在 `robot`、`human`、`animal`、`plant`、`fish`、`geo` 之间切换
- 实时预览生成结果
- 调整配置并同步到 URL
- 导出 SVG、PNG 或 WebP
- 复制配置 JSON
...
export function Example() {
  return (
    <AgentFace
      ...
      characterType="fish"
      size={120}
      imageUrl="https://example.com/avatar.png" // 选填
      imageAlt="Custom avatar"
      loadingShowcaseMode="skeleton" // skeleton/avatar（选填）
...
如果传入 `imageUrl`，组件会在加载期间显示轻量占位图，并在图片加载失败时自动回退到 AgentFace。不传 `characterType` 时，行为会保持和旧版 robot 渲染完全兼容。
