...
import { CHARACTER_TYPES, CHARACTER_TYPE_OPTIONS } from "@agent-face/core";
...
type ConfigPanelProps = {
  ...
  values: Partial<Record<AgentFaceOptionKey, string>>;
...
function ConfigField({
  ...
  const options = CHARACTER_TYPE_OPTIONS[characterType][field as never] as readonly string[];
    ...
    <Card className="border-border/70 bg-card/90 backdrop-blur">
      ...
      <CardContent>
        <div className="mb-4 space-y-3 rounded-[calc(var(--radius)-0.15rem)] border border-border/70 bg-background/80 p-4">
          <div>
            <p className="text-sm font-semibold">{copy.characterType.label}</p>
            <p className="mt-1 text-xs text-muted-foreground">{copy.characterType.hint}</p>
          </div>

          <Select onValueChange={(value) => onCharacterTypeChange(value as CharacterType)} value={characterType}>
            <SelectTrigger>
              <SelectValue placeholder={copy.characterType.label} />
            </SelectTrigger>
            <SelectContent>
              {CHARACTER_TYPES.map((option) => (
                <SelectItem key={option} value={option}>
                  {copy.characterType.options[option]}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
        ...
        <Accordion
          className="space-y-3"
          defaultValue={copy.groups[characterType].map((group) => group.id)}
          type="multiple"
        >
          {copy.groups[characterType].map((group) => (
